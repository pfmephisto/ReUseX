...
void setup_subcommand_export_semantic_images(
  ...
  auto *sub = parent.add_subcommand(
      "semantic-images", "Export semantic segmentation images as colored PNGs");
...
int run_subcommand_export_semantic_images(
  ...
  spdlog::info("Exporting semantic images from project: {}",
               project_path.string());
  ...
  try {
    ...
    spdmon::LoggerProgress progress("Exporting",
    ...
    for (int id : ids) {
      ...
      labels.forEach<int32_t>([&](int32_t label, const int pos[]) {
        ...
        auto c = pcl::GlasbeyLUT::at(static_cast<unsigned>(label) %
                                     pcl::GlasbeyLUT::size());
        colored.at<cv::Vec3b>(pos[0], pos[1]) =
            cv::Vec3b(c.b, c.g, c.r); // OpenCV is BGR
      });

      std::ostringstream id_str;
      id_str << std::setw(pad_width) << std::setfill('0') << id;
      fs::path out_path =
          opt.output_dir / (opt.prefix + "_" + id_str.str() + ".png");
    ...
    spdlog::info("Exported {} semantic image(s) to {}", exported,
