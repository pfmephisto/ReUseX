...
void VizualizationObserver::viewer_add_plane(std::string_view name,
  ...
  if (std::string_view(name).find("horizontal_planes") !=
      std::string_view::npos) {
    auto centroid = plane.second;
    double height = centroid.z();
    // Use ViridisLUT color based on index
    auto color =
        pcl::ViridisLUT::at(static_cast<size_t>(idx) % pcl::ViridisLUT::size());

    viewer_enqueue_task([name = std::string(name), centroid, height, color, vp](
                            const ViewerPtr &viewer, const std::vector<int> &) {
      // Create a bounded floor quad centered on the centroid XY
      constexpr double extent = 5.0;
      pcl::PointCloud<pcl::PointXYZ>::Ptr points(
          new pcl::PointCloud<pcl::PointXYZ>);
      points->push_back(
          pcl::PointXYZ(centroid.x() - extent, centroid.y() - extent, height));
      points->push_back(
          pcl::PointXYZ(centroid.x() + extent, centroid.y() - extent, height));
      points->push_back(
          pcl::PointXYZ(centroid.x() + extent, centroid.y() + extent, height));
  ...
  viewer_enqueue_task([name = std::string(name), cc,
    ...
    if (name.find("room_prob") != std::string::npos) {
      ...
      for (auto fit = cc->faces_begin(); fit != cc->faces_end(); ++fit) {
        ...
        auto plane_id =
            std::get<reusex::geometry::FaceData>((*cc)[*fit].data).plane_id;
...
void VizualizationObserver::viewer_add_camera_frustum(
  ...
  viewer_enqueue_task(
      [name = std::string(name), focal_x, focal_y, image_width, image_height,
       pose](const ViewerPtr &viewer, const std::vector<int> &viewports) {
        int vp = viewports.empty() ? 0 : viewports[0];
...
void VizualizationObserver::viewer_add_visibility_graph(
  ...
  viewer_enqueue_task([name = std::string(name), disc_points,
    ...
    viewer->setShapeRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_COLOR, 0.1, 0.3, 0.9, edges_name,
        vp);
  });
}
