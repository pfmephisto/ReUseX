...
struct LayoutConfig {
  ...
  int maxIterations = 30; // Slightly increase the number of iterations to
                          // ensure the optimal solution among multiple anchors
  int paddingX = 2;
  int paddingY = 2;

  // --- Core anchor costs (corresponding to 1, 2, 3, 4) ---
  // The difference values here (e.g., 0, 10, 20, 30) determine the
  // 'sensitivity' of the algorithm to switch positions
  ...
  // --- Penalty terms ---
  // Must be significantly greater than the highest priority anchor cost (30.0)
  // to ensure 'sliding only starts when necessary'
  float costSlidingPenalty = 100.0f;

  // Font size scaling penalty: keep a very large value to ensure changing
  // position is preferred over reducing font size
  float costScaleTier = 10000.0f;

  // Occlusion/overlap penalty: keep a very large value, once collision occurs,
  // the cost will quickly exceed the sliding penalty
...
class LabelLayoutSolver {
    ...
    for (int iter = 0; iter < config.maxIterations; ++iter) {
      ...
      for (int idx : processOrder) {
        ...
        for (int i = 0; i < (int)item.candCount; ++i) {
          ...
          // Heuristic pruning
          // If the base cost already exceeds the current optimal, no need to
          // perform dynamic overlap calculation
  ...
  void generateCandidatesInternal(LayoutItem &item, const std::string &text,
    ...
    for (const auto &lvl : levels) {
      ...
      // Helper function: calculate step size based on edge length, ensure
      // sampling every ~20-50 pixels, at least 3 steps, at most 15 steps
      ...
      if (rangeX > 1.0f) {
        ...
        for (int i = 1; i < stepsX;
             ++i) { // 1 to steps-1, avoid existing anchors
          float r = i * invStepsX;
