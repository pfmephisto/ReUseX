...
auto CellComplex::compute_room_probabilities(
...

  {
    ...
    auto observer = ReUseX::core::ProgressObserver(
        "Computing room probabilities", this->num_cells());
...
#pragma omp parallel for reduction(+ : accum_ptr[ : N])
      for (size_t i = 0; i < dirs.size(); ++i) {
        ...
        if (rayhit.hit.geomID == RTC_INVALID_GEOMETRY_ID) { // No hit
          accum_ptr[0] += 1;
          // ReUseX::core::trace(fmt::format(fmt::fg(fmt::color::red), "No
          // hit"));
          continue;
        }
        ...
        // ReUseX::core::trace(fmt::format(fmt::fg(fmt::color::green),
        // "Frontside hit"));
      ...
      // 0.0); ReUseX::core::trace("Cell {:>3} sum = {:.3f}", idx, sum); if (sum
      // > 0)
  ...
  ReUseX::core::trace("Sum probabilities => [{:.3f}]",
