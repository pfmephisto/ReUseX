...
class Visualizer {
   ...
   * @param vp Viewport index. Default 0.
   */
  void add_floors(const std::vector<double> &heights,
                  const Eigen::Vector3d &min, const Eigen::Vector3d &max,
                  const std::string_view &name = "floor", int vp = 0);

  void
  add_cell_complex(const std::shared_ptr<ReUseX::geometry::CellComplex> &cc,
  ...
  void
  add_rooms(const std::shared_ptr<ReUseX::geometry::CellComplex> &cc,
            const std::pair<
  ...
  addRoomProbabilities(const std::shared_ptr<ReUseX::geometry::CellComplex> &cc,
