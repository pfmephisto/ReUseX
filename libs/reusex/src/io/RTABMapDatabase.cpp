...
public:
    ...
    Impl(std::filesystem::path path, bool ro)
        ...
        if (sqlite3_open_v2(dbPath.string().c_str(), &db, flags, nullptr) != SQLITE_OK) {
        ...
        if (!driver->openConnection(dbPath.string(), readOnly)) {
            delete driver;
            sqlite3_close(db);
            throw std::runtime_error("DBDriver failed to open connection to database");
        }
        ...
        try {
            ...
            delete driver;
            sqlite3_close(db);
            throw std::runtime_error(std::string("Rtabmap failed to initialize: ") + e.what());
        }
    ...
    void validateSchema() const {
        ...
        for (const char* table : tables) {
            ...
            if (sqlite3_prepare_v2(db, query.c_str(), -1, &stmt, nullptr) != SQLITE_OK) {
                throw std::runtime_error("Schema validation failed: " + std::string(sqlite3_errmsg(db)));
            }
    ...
    std::vector<int> getNodeIds(bool ignoreChildren) const {
        ...
        std::string query = ignoreChildren
            ? "SELECT id FROM Node WHERE time_enter >= 0.0;"  // Parent nodes only
            : "SELECT id FROM Node;";  // All nodes

        sqlite3_stmt* stmt;
        if (sqlite3_prepare_v2(db, query.c_str(), -1, &stmt, nullptr) != SQLITE_OK) {
            throw std::runtime_error("Failed to query node IDs: " + std::string(sqlite3_errmsg(db)));
        }
    ...
    cv::Mat getImage(int nodeId) const {
        ...
        if (sqlite3_prepare_v2(db, query, -1, &stmt, nullptr) != SQLITE_OK) {
            throw std::runtime_error("Failed to prepare image query: " + std::string(sqlite3_errmsg(db)));
        }
    ...
    cv::Mat getLabels(int nodeId) const {
        ...
        if (sqlite3_prepare_v2(db, query, -1, &stmt, nullptr) != SQLITE_OK) {
            throw std::runtime_error("Failed to prepare labels query: " + std::string(sqlite3_errmsg(db)));
        }
    ...
    void saveLabels(int nodeId, const cv::Mat& labels, bool autoRotate) {
        ...
        sqlite3_bind_int(stmt, 1, nodeId);
        sqlite3_bind_blob(stmt, 2, bytes.data(), static_cast<int>(bytes.size()), SQLITE_STATIC);
...
RTABMapDatabase& RTABMapDatabase::operator=(RTABMapDatabase&&) noexcept = default;
...
void RTABMapDatabase::validateSchema() const {
    impl_->validateSchema();
}
...
void RTABMapDatabase::saveLabels(int nodeId, const cv::Mat& labels, bool autoRotate) {
