...
static std::array<double, 12> pose_to_matrix(const e57::RigidBodyTransform &t) {
  ...
  return {
      ...
      t.translation.x,         2 * (x * y + w * z),     1 - 2 * (x * x + z * z),
      2 * (y * z - w * x),     t.translation.y,         2 * (x * z - w * y),
      2 * (y * z + w * x),     1 - 2 * (x * x + y * y), t.translation.z,
  };
}

static void apply_pose(const std::array<double, 12> &T, float &px, float &py,
                       float &pz) {
...
static bool is_identity_pose(const e57::RigidBodyTransform &t) {
  return t.rotation.w == 1.0 && t.rotation.x == 0.0 && t.rotation.y == 0.0 &&
         t.rotation.z == 0.0 && t.translation.x == 0.0 &&
         t.translation.y == 0.0 && t.translation.z == 0.0;
}
...
void import_e57(ProjectDB &db, const std::filesystem::path &e57_path) {
  ...
  int logId = db.log_pipeline_start(
      "import_e57", fmt::format(R"({{"source":"{}"}})", e57_path.string()));
  ...
  try {
    ...
    for (int64_t scanIdx = 0; scanIdx < scanCount; ++scanIdx) {
      ...
      if (!fields.cartesianXField || !fields.cartesianYField ||
          !fields.cartesianZField) {
        core::warn("Scan {}: no Cartesian XYZ fields, skipping", scanIdx);
        continue;
      }

      int64_t rowMax = 0, colMax = 0, pointsSize = 0, groupsSize = 0,
              countSize = 0;
      bool colIndex = false;
      reader.GetData3DSizes(scanIdx, rowMax, colMax, pointsSize, groupsSize,
                            countSize, colIndex);
      ...
      bool hasColor = fields.colorRedField && fields.colorGreenField &&
                      fields.colorBlueField;
      bool hasNormals =
...
void export_e57(const std::filesystem::path &path,
  ...
  for (const auto &scan : scans) {
    ...
    core::info("Writing scan '{}': {} points  normals={}", scan.name, N,
               hasNormals);
