...
static bool has_field(const pcl::PCLPointCloud2 &cloud2,
...
void import_ply(ProjectDB &db, const std::filesystem::path &ply_path) {
  ...
  int logId = db.log_pipeline_start(
      "import_ply", fmt::format(R"({{"source":"{}"}})", ply_path.string()));
  ...
  try {
    ...
    if (cloud2.width == 0 || cloud2.height == 0) {
      throw std::runtime_error("PLY file contains no points: " +
                               ply_path.string());
    }
    ...
    bool hasXYZ = has_field(cloud2, "x") && has_field(cloud2, "y") &&
                  has_field(cloud2, "z");
    bool hasRgb = has_field(cloud2, "rgb") || has_field(cloud2, "rgba");
    bool hasRgbSep = has_field(cloud2, "r") && has_field(cloud2, "g") &&
                     has_field(cloud2, "b");
    bool hasColor = hasRgb || hasRgbSep;
    bool hasNormals =
        (has_field(cloud2, "normal_x") || has_field(cloud2, "nx")) &&
    ...
    if (!hasXYZ) {
      throw std::runtime_error("PLY file has no XYZ position fields: " +
                               ply_path.string());
    }
      ...
      for (size_t i = 0; i < cloud.size(); ++i) {
        if (i < xyzi_cloud.size() && std::isfinite(xyzi_cloud[i].intensity)) {
          auto gray = static_cast<uint8_t>(
              std::clamp((xyzi_cloud[i].intensity - iMin) / iRange * 255.0f,
                         0.0f, 255.0f));
    ...
    if (hasNormals) {
      ...
      bool hasData =
          std::any_of(normals.begin(), normals.end(), [](const NormalT &n) {
            return std::isfinite(n.normal_x) &&
                   (n.normal_x != 0.0f || n.normal_y != 0.0f ||
                    n.normal_z != 0.0f);
          });

      if (hasData) {
        std::string normalsName = cloudName + "_normals";
        core::info("Saving normals cloud '{}' ({} normals)", normalsName,
                   normals.size());
    ...
    if (hasLabel) {
      ...
      if (hasData) {
        ...
        core::info("Saving label cloud '{}' ({} labels)", labelsName,
                   labels.size());
