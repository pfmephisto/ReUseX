#include <ReUseX/vision/common/affine.hpp>
...
#include <ReUseX/vision/kernels/postprocess.cuh>
#include <ReUseX/vision/kernels/process_kernel_warp.hpp>
#include <algorithm>
...
std::shared_ptr<InferBase> load(const std::string &vision_encoder_path,
  ...
  return Sam3Infer::create_instance(vision_encoder_path, text_encoder_path,
                                    decoder_path, gpu_id);
}

// Static factory function
std::shared_ptr<Sam3Infer>
Sam3Infer::create_instance(const std::string &vision_encoder_path,
                           const std::string &text_encoder_path,
                           const std::string &decoder_path, int gpu_id) {
...
Sam3Infer::Sam3Infer(const std::string &vision_encoder_path,
                     const std::string &text_encoder_path,
                     const std::string &geometry_encoder_path,
                     const std::string &decoder_path, int gpu_id)
    : InferBase(), vision_encoder_path_(vision_encoder_path),
...
void Sam3Infer::setup_text_inputs(
...
// Only call geometry model to store results in geom_features_cache_ and geom_mask__cache_
bool Sam3Infer::setup_geometry_input(
...
void Sam3Infer::allocate_memory_once() {
  ...
  // Mask Postprocess (assume each prompt produces at most one valid mask to estimate buffer)
  // Or directly allocate enough GPU memory. For safety, allocate by max_prompt_batch_ * num_queries_
  // Allocation: usually only a few valid boxes. Allocate a safe value, such as
  // 256MB, or check dynamically. Here allocate the maximum possible. Assume each prompt produces 1 mask, maximum
  // max_prompt_batch_ masks. If many masks, process in a loop. Here,
...
void Sam3Infer::set_binding_dim(std::shared_ptr<TensorRT::Engine> &engine,
...
void Sam3Infer::gather_vision_features(
  ...
  for (int i = 0; i < batch_size; ++i) {
    ...
    // Asynchronous copy
    // The number of copies here equals batch_size, usually < 100, overhead is controllable.
    // For ultimate optimization, a Kernel can be written, but it's simpler to maintain in C++ logic
...
bool Sam3Infer::encode_text(const std::vector<PromptMeta> &batch_prompts,
...
bool Sam3Infer::encode_boxes(const std::vector<PromptMeta> &batch_prompts,
...
void Sam3Infer::postprocess(InferResult &image_result, int batch_idx,
...
InferResultArray Sam3Infer::forwards(const std::vector<Sam3Input> &inputs,
  ...
  if (geom_mask_cache_.count(geom_label) == 0 ||
    ...
    std::cerr << "Geometry cache not found for label: " << geom_label
              << std::endl;
    return {};
  }
  ...
  if (inputs.size() > (size_t)max_image_batch_) {
    std::cerr << "Input image batch size (" << inputs.size()
              << ") exceeds maximum supported (" << max_image_batch_
  ...
  // Get pointer to cached data (assume cache stores result for batch=1 at start of GPU memory)
...
InferResultArray Sam3Infer::forwards(const std::vector<Sam3Input> &inputs,
  ...
  // If actual Box count exceeds preset GPU memory allocation, truncate (prevent overflow)
  ...
  for (int chunk_start = 0; chunk_start < total_prompts;
    ...
    // a. Gather Vision Features (gather from N images' features to M Prompts in current batch
    // Prompt features)
