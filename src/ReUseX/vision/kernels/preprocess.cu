#include <ReUseX/vision/kernels/preprocess.cuh>
...
__global__ void warp_affine_bilinear_and_normalize_plane_kernel(
  ...
  if (norm.channel_type == ReUseX::vision::norm_image::ChannelType::SwapRB) {
    float t = c2;
    c2 = c0;
    c0 = t;
  }

  if (norm.type == ReUseX::vision::norm_image::NormType::MeanStd) {
    c0 = (c0 * norm.alpha - norm.mean[0]) / norm.std[0];
    c1 = (c1 * norm.alpha - norm.mean[1]) / norm.std[1];
    c2 = (c2 * norm.alpha - norm.mean[2]) / norm.std[2];
  } else if (norm.type == ReUseX::vision::norm_image::NormType::AlphaBeta) {
...
__global__ void warp_affine_bilinear_single_channel_kernel(
  ...
  if (src_x < 0 || src_x >= src_width || src_y < 0 || src_y >= src_height) {
    ...
    float *v1 =
        (y_low >= 0 && y_low < src_height && x_low >= 0 && x_low < src_width)
            ? src + y_low * src_line_size + x_low
            : &const_value_st;
    float *v2 =
        (y_low >= 0 && y_low < src_height && x_high >= 0 && x_high < src_width)
            ? src + y_low * src_line_size + x_high
            : &const_value_st;
    float *v3 =
        (y_high >= 0 && y_high < src_height && x_low >= 0 && x_low < src_width)
            ? src + y_high * src_line_size + x_low
            : &const_value_st;
    float *v4 = (y_high >= 0 && y_high < src_height && x_high >= 0 &&
                 x_high < src_width)
...
__global__ void warp_affine_bilinear_single_channel_mask_kernel(
  ...
  dst[dy * dst_width + dx] = (c0 > 0.0f) ? 255 : 0;
}
