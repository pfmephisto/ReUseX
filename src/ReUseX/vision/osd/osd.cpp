...
void overlay_mask(cv::Mat &image, const cv::Mat &mask,
                  const ::ReUseX::vision::object::Box &box,
...
static const char *font_path = "./DejaVuSansMNerdFont-Regular.ttf";
static ReUseX::vision::CvxText text_renderer(font_path);
} // namespace
...
void drawBaseInfoGeometry(cv::Mat &img,
                          const ::ReUseX::vision::object::DetectionBox &box,
...
void drawPoseSkeleton(cv::Mat &img,
                      const ::ReUseX::vision::object::DetectionBox &box,
                      int thickness) {
  if (box.type == ::ReUseX::vision::object::ObjectType::TRACK || !box.pose)
    return;
...
void drawObbBox(cv::Mat &img, const ::ReUseX::vision::object::DetectionBox &box,
                int thickness) {
  if (box.type == ::ReUseX::vision::object::ObjectType::TRACK || !box.obb)
    return;
...
void drawTrackTrace(cv::Mat &img,
                    const ::ReUseX::vision::object::DetectionBox &box,
                    int font_size) {
  if (!box.track)
    return;
  ...
  text_renderer.putText(img, text,
...
int calculateDynamicFontSize(int img_w, int img_h,
                             const ::ReUseX::vision::object::Box &box,
                             double ratio) {
...
void osd(cv::Mat &img, const ::ReUseX::vision::object::DetectionBoxArray &boxes,
  ...
  for (const auto &box : boxes) {
    if (box.type == ::ReUseX::vision::object::ObjectType::DEPTH_PRO ||
        box.type == ::ReUseX::vision::object::ObjectType::DEPTH_ANYTHING) {
      drawDepth(img, box);
      continue;
    }
    ...
    if (osd_rect) {
      if (box.type == ::ReUseX::vision::object::ObjectType::POSITION)
...
void osd(
  ...
  for (const auto &[l, pts] : points) {
    ...
    if (pts.size() >= 3) {
      ...
      text_renderer.putText(img, l, cv::Point(std::get<0>(c), std::get<1>(c)),
                            color, fs);
    }
  }
}
...
void osd(cv::Mat &img, const std::string &fence_name,
  ...
  if (points.size() >= 3) {
    ...
    text_renderer.putText(img, fence_name,
