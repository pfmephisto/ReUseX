...
// This is the most important modification. It no longer uses a rigid switch
// structure, Instead, it prints all required fields first, then checks each
// optional member for existence. This approach is more flexible and can
// accurately reflect any combination of data.
//================================================================================
std::ostream &operator<<(std::ostream &os, const DetectionBox &box) {
  ...
  if (box.segmentation.has_value()) {
    // Directly printing the entire mask matrix is impractical, so we only print
...
cv::Mat segmentMapToMat(const std::shared_ptr<SegmentMap> &map) {
  ...
  // Use SegmentMap's data pointer, height, and width to create a cv::Mat object
  // header. CV_8UC1 means this is an 8-bit unsigned single-channel image, which
  // is usually the format for segmentation masks. This cv::Mat constructor does
  // not copy data, but directly uses the provided pointer.
