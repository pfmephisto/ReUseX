...
void ReUseX::PointCloud::clustering(double cluster_tolerance,
                                    pcl::uindex_t min_cluster_size,
                                    pcl::uindex_t max_cluster_size) {

  using Filter = pcl::experimental::advanced::FunctorFilter<
      PointCloud::Cloud::PointType,
      MatchCondition<PointCloud::Cloud::PointType>>;
  ...
  pcl::search::KdTree<PointCloud::Cloud::PointType>::Ptr tree(
      new pcl::search::KdTree<PointCloud::Cloud::PointType>);
  tree->setInputCloud(cloud);

  auto monitor = spdmon::LoggerProgress("Clustering", lables_vec.size());
  spdlog::stopwatch sw;
...
#pragma omp parallel for shared(cloud, lables_vec) private(tree)
  ...
  spdlog::info("Clustering took {} seconds", sw);
}
