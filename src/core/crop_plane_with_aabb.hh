...
#include "get_csystem.hh"

#include <CGAL/boost/graph/Euler_operations.h>
#include <cmath>

static auto comparator = [](std::pair<float, ReUseX::Point> p1,
                            std::pair<float, ReUseX::Point> p2) {
...
get_points(std::array<ReUseX::Line, 12> const &segments,
           ReUseX::Plane const &plane) {
...
static std::vector<float> get_angles_in_plane(Eigen::Matrix3f mat,
                                              std::vector<ReUseX::Point> points,
                                              ReUseX::Point center) {

  auto angs = std::vector<float>();
  for (auto &p : points) {
    auto v = Eigen::Vector3f(p.x() - center.x(), p.y() - center.y(),
                             p.z() - center.z())
                 .normalized();
    v = mat.inverse() * v;
...
static void crop_plane_with_aabb(Mesh &mesh, const Box &box,
                                 const Plane plane) {
  ...
  for (auto &p : pairs)
    vertecies.push_back(
        mesh.add_vertex(Point(p.second.x(), p.second.y(), p.second.z())));
