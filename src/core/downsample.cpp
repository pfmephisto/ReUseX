...
void downsample(PointCloud::Cloud::Ptr cloud, double leaf_size) {
  ...
  pcl::octree::OctreePointCloudPointVector<PointCloud::Cloud::PointType> octree(
      leaf_size);
  octree.setInputCloud(cloud);
  octree.addPointsFromInputCloud();

  std::vector<pcl::octree::OctreePointCloudPointVector<
      PointCloud::Cloud::PointType>::LeafNodeIterator>
      nodes;
  ...
  for (size_t i = 0; i < leaf_count; i++) {
    ...
    for (auto const &index : indexVector) {
      ...
      boost::fusion::for_each(
          acc, pcl::detail::AddPoint<PointCloud::Cloud::PointType>(point));
    }

    boost::fusion::for_each(acc,
                            pcl::detail::GetPoint<PointCloud::Cloud::PointType>(
                                filtered_cloud->at(i), indexVector.size()));
  }
