...
void inflate(Eigen::MatrixXd &in, double inflate_factor) {
  auto lam = [inflate_factor](double x) -> double {
    return std::pow(x, inflate_factor);
  };
  in = in.unaryExpr(lam);
  in = normalize(in);
}
...
template <>
mcl<MCLAlgorithm::CPP, Eigen::MatrixXd>::mcl(
    Eigen::MatrixXd &Min,
...
*/
template <>
void mcl<MCLAlgorithm::CPP, Eigen::MatrixXd>::cluster_mcl(double expand_factor,
                                                          double inflate_factor,
                                                          double max_loop,
                                                          double mult_factor) {
  Eigen::MatrixXd M_selfloop =
      Data +
      (mult_factor * Eigen::MatrixXd::Identity(Data.cols(), Data.rows()));
