...
// using MIP_Solver            =
// CGAL::SCIP_mixed_integer_program_traits<double>; #endif

// Custom reduction function for std::unordered_set<int>
inline void merge_sets(std::unordered_set<int> &lhs,
                       const std::unordered_set<int> &rhs) {
  for (auto it = rhs.begin(); it != rhs.end(); it++)
    lhs.insert(*it);
}

#pragma omp declare reduction(merge : std::unordered_set<int> : merge_sets(    \
        omp_out, omp_in)) initializer(omp_priv = std::unordered_set<int>())

namespace ReUseX {
template <typename PointT> struct PointMap {
  ...
  friend void put(const PointMap &, PointCloud::Cloud::PointType &k,
                  const PointT &v) {
...
template <typename VectorT> struct NormalMap {
  ...
  friend void put(const NormalMap &, PointCloud::Cloud::PointType &k,
                  const VectorT &v) {
...
struct PlaneIndexMap {
  ...
  const int &operator[](const PointCloud::Cloud::PointType &point) const {
    return point.label;
  }

  friend int get(const PlaneIndexMap &, const PointCloud::Cloud::PointType &k) {
    return k.label;
  }
  friend void put(const PlaneIndexMap &, PointCloud::Cloud::PointType &k,
                  const int &v) {
    k.label = (std::int32_t)v;
  }
};

static std::vector<pcl::PointIndices::Ptr>
make_single_cluster(typename pcl::PointCloud<PointT>::ConstPtr cloud) {
...
template <typename Mesh> void repair(Mesh &mesh) {
...
template<typename Mesh>
  ...
  auto face_map =
      mesh.template add_property_map<typename Mesh::Face_index, int>(
              "f:component")
          .first;
...
void create_polysurface(typename pcl::PointCloud<PointT>::ConstPtr cloud,
                        pcl::PointIndices::ConstPtr sellection, Mesh &mesh,
                        size_t i = 0, double fitting = 0.20,
  ...
  // Since CGAL explects planes to be number contiguously from 0, we need to
  // renumber the planes The plane indecies need to be rempated.
...
#pragma omp critical(print)
    ...
    int i = 0;
    std::unordered_map<int, int> plane_index_map =
        std::unordered_map<int, int>();
    for (auto it = plane_indices_set.begin(); it != plane_indices_set.end();
         it++)
...
#pragma omp parallel for
    for (size_t i = 0; i < cluster_cloud->points.size(); i++)
      cluster_cloud->points[i].label =
          plane_index_map[cluster_cloud->points[i].label];

    printf("Computing poly surface for Task %d with %d planes.\n", i,
           plane_indices_set.size());
    CGAL::Polygonal_surface_reconstruction<Kernel> psr(
        cluster_cloud->points, PointMap<Point>(), NormalMap<Vector>(),
        PlaneIndexMap());
    psr.template reconstruct<MIP_Solver>(
        mesh, 0.20 /*fitting*/, 0.10 /*coverage*/, 0.70 /*complexity*/);
...
solidify(typename pcl::PointCloud<PointT>::ConstPtr cloud,
         std::optional<std::vector<pcl::PointIndices::Ptr>> clusters_in =
             std::nullopt,
         double fitting = 0.20, double coverage = 0.10,
  ...
  std::vector<pcl::PointIndices::Ptr> clusters =
      (clusters_in.has_value()) ? clusters_in.value()
                                : make_single_cluster(cloud);

  std::cout << "Number of clusters: " << clusters.size() << std::endl;

  std::vector<Mesh> meshes = std::vector<Mesh>(clusters.size());
  auto monitor = spdmon::LoggerProgress("Solidification", clusters.size());
  spdlog::stopwatch sw;
...
#else
  for (size_t i = 0; i < clusters.size(); i++) {
    create_polysurface<PointT, Mesh>(cloud, clusters[i], meshes[i], i, fitting,
                                     coverage, complexity);
    ++monitor;
  }
#endif
  spdlog::info("Solidification took {} seconds", sw);

  // Filter meshes with no faces and too small volumes
  std::vector<Mesh> meshes_temp = std::vector<Mesh>();
  std::copy_if(meshes.begin(), meshes.end(), std::back_inserter(meshes_temp),
               is_vaild<Mesh>);
