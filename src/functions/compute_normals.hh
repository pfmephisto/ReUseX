 ...
 *
 * This function takes a container of input point clouds, computes the normals
 * for each point cloud, and returns a container of point clouds with the
 * computed normals concatenated to the original points.
 *
 * @tparam PointTIn The type of the input point cloud points.
 * @tparam PointTOut The type of the output point cloud points.
 * @tparam InAllocator The allocator type for the input container. Defaults to
 * std::allocator.
 * @tparam InContainer The container type for the input point clouds. Defaults
 * to std::vector.
 * @tparam OutContainer The container type for the output point clouds. Defaults
 * to std::vector with Eigen::aligned_allocator.
 *
 * @param src The container of input point clouds.
 * @return OutContainer The container of output point clouds with computed
 * normals.
 */
template <
    typename PointTIn, typename PointTOut,
    typename InAllocator =
        std::allocator<typename pcl::PointCloud<PointTIn>::Ptr>,
    typename InContainer =
        std::vector<typename pcl::PointCloud<PointTIn>::Ptr, InAllocator>,
    typename OutContainer = std::vector<
        typename pcl::PointCloud<PointTOut>::Ptr,
        Eigen::aligned_allocator<typename pcl::PointCloud<PointTOut>::Ptr>>>
 ...
 *
 * This function takes an input point cloud and computes the normals for each
 * point in the cloud. It uses the PCL (Point Cloud Library) for normal
 * estimation and concatenates the computed normals with the input cloud to
 * produce an output cloud that contains both the original points and their
 ...
 * @return PointTOut::Ptr The output point cloud containing both the original
 * points and their computed normals.
 */
template <typename PointT_In, typename PointT_Out>
static typename pcl::PointCloud<PointT_Out>::Ptr
compute_normals(typename pcl::PointCloud<PointT_In>::Ptr cloud_in) {
 ...
 *
 * This function takes a vector of file paths, loads the point clouds from these
 * files, computes the normals for each point cloud, and then saves the
 * resulting point clouds with normals back to the respective files.
 *
 * @tparam PointTIn  The type of the input point cloud.
...
static void compute_normals(std::vector<fs::path> paths) {
  ...
  using OutContainer = std::vector<
      typename pcl::PointCloud<PointTOut>::Ptr,
      Eigen::aligned_allocator<typename pcl::PointCloud<PointTOut>::Ptr>>;
  using InContainer = std::vector<
      typename pcl::PointCloud<PointTIn>::Ptr,
      Eigen::aligned_allocator<typename pcl::PointCloud<PointTIn>::Ptr>>;

  // Load the clouds and compute normals
  InContainer clouds_in;
  std::transform(paths.begin(), paths.end(), std::back_inserter(clouds_in),
                 [](fs::path const &file) { return load<PointTIn>(file); });
...
/**
 * @brief Computes normals for a set of point cloud files in the specified
 * directory.
 *
 * This function iterates through the given directory, collects all files with a
 * ".pcd" extension, sorts them, and then computes normals for each file.
 *
