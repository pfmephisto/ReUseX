...
static void read_results(const std::string &filename,
                         std::function<void(size_t cls_i, size_t mem_j)> f) {
...
template <>
mcl<MCLAlgorithm::CLI, Eigen::MatrixXd>::mcl(
    Eigen::MatrixXd &Min,
    std::function<void(size_t cluster_j, size_t member_i)> f)
    : Data(Min), ClusterResultCallback(f) {}

template <>
mcl<MCLAlgorithm::CLI, const RayCollection>::mcl(
    const RayCollection &rays,
    std::function<void(size_t cluster_j, size_t member_i)> f)
    : Data(rays), ClusterResultCallback(f) {}

/*! @brief Apply Markov clustering algorithm with specified parameters and
return clusters For each cluster, returns the list of node-ids that belong to
that cluster
*/
template <>
void mcl<MCLAlgorithm::CLI, Eigen::MatrixXd>::cluster_mcl(double expand_factor,
  ...
  try {
    ...
    std::stringstream ss;
    int threads = 30;
    ss << "mcl " << input_path << " --abc -I " << inflate_factor << " -te "
       << threads << " -o " << output_path;
...
template <>
void mcl<MCLAlgorithm::CLI, const RayCollection>::cluster_mcl(
    double expand_factor, double inflate_factor, double max_loop,
    double mult_factor) {
  try {
    write_file(input_path, Data);
    std::stringstream ss;
