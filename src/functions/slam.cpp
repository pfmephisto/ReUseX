...
#include "polyscope.hh" // While debugging
...
namespace ReUseX
{
    int slam(fs::path const & directory){
        ...
        // Load the clouds
        std::vector<pcl::PointCloud<PointT>::Ptr, Eigen::aligned_allocator<pcl::PointCloud<PointT>::Ptr>> clouds;
        std::transform(files.begin(), files.end(), std::back_inserter(clouds), [](const auto& file){ return load<PointT>(file);});


        // Calculate Registartion
        ///////////////////////////////////////////////////////////////////////////////
        auto transforms = std::vector<Eigen::Matrix4f>(clouds.size(), Eigen::Matrix4f::Identity());
        ...
        ///////////////////////////////////////////////////////////////////////////////
        auto update_bar = util::progress_bar(clouds.size(),"Compute compound transformations");
        auto compund_transforms = std::vector<Eigen::Matrix4f>(clouds.size(), Eigen::Matrix4f::Identity());
        for (std::size_t i = 1; i < clouds.size() ; ++i){
        ...
        for (std::size_t i = 1; i < clouds.size() ; ++i){
            pcl::transformPointCloudWithNormals(*clouds[i], *clouds[i], compund_transforms[i]);
            move_bar.update();
        }
        ...
        for (size_t i = 0; i < clouds.size(); ++i)
            polyscope::display<pcl::PointCloud<PointT> const&>(*clouds[i], fmt::format("Cloud {}", clouds[i]->header.frame_id).c_str());
        polyscope::myshow();
        ...
        fmt::print("Number of clouds: {} in {} secons.\n", clouds.size(), std::chrono::duration<double>(duration).count());
