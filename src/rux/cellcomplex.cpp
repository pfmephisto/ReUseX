...
#include <ReUseX/force_orthogonal_planes.hpp>
#include <ReUseX/helpers.hpp>
#include <ReUseX/io.hpp>
#include <ReUseX/make_pairs.hpp>
#include <ReUseX/merge_planes.hpp>
...
int run_subcommand_cellcomplex(SubcommandCellcomplexOptions const &opt) {
  ...
  ReUseX::read(planes_path, planes_coeff, centroids_in, inliers);
  ...
  planes = ReUseX::force_orthogonal_planes(planes);
  spdlog::debug("Number of planes after forcing orthogonality: {}",
                planes.size());

  const double threshold = 1.5;
  const double new_plane_offset = 0.5;
  auto pairs = ReUseX::make_pairs(planes, inliers, centroids, threshold,
                                  new_plane_offset);
  spdlog::debug("Number of plane pairs: {}", pairs.size());

  auto [vertical, horizontal] = ReUseX::separate_planes(planes);
  ...
  if (opt.display) {
    ...
    for (size_t i = 0; i < pairs.size(); ++i) {
      ...
      viewer->addText3D(fmt::format("{}", pairs[i].first),
                        ...
                        0.1, 1.0, 1.0, 1.0,
                        fmt::format("text_{}_{}", name, pairs[i].first), vp_1);
  ...
  auto viz_callback =
      [&viewer, vp_2](size_t idx, std::vector<std::array<double, 3>> const &pts,
  ...
  if (opt.display) {
    ...
    viewer->addPointCloud<PointT>(cells, c_color_handler, c_name);
    viewer->setPointCloudRenderingProperties(
  ...
  // std::optional<
  //     std::pair<std::unordered_map<ReUseX::CellComplex::Vertex, int>,
  //               std::unordered_map<ReUseX::CellComplex::Vertex,
  //               std::set<int>>>>
  //     results({});
