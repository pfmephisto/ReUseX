...
#pragma omp parallel for reduction(min : x_min, y_min, z_min)                  \
    reduction(max : x_max, y_max, z_max)
...
std::vector<Brep> PointCloud::solidify(unsigned int downsample_size, double sx,
                                       double sy, double expand_factor,
                                       double inflate_factor, double max_loop,
                                       double mult_factor, double fitting,
  ...
  auto clusters = ReUseX::cluster_rooms<PointCloud::Cloud::PointType>(
      cloud_copy, downsample_size, sx, sy, expand_factor, inflate_factor,
      max_loop, mult_factor);
  ...
  auto meshes = ReUseX::solidify<PointCloud::Cloud::PointType>(
      cloud, clusters, fitting, coverage, complexity);

  std::vector<Brep> breps;
  std::transform(meshes.begin(), meshes.end(), std::back_inserter(breps),
                 [](Mesh const &mesh) { return Brep(mesh); });
