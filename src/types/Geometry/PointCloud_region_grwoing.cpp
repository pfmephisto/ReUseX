...
void ReUseX::PointCloud::region_growing(float angle_threshold,
                                        float plane_dist_threshold,
                                        int minClusterSize, float early_stop,
                                        float radius, float interval_0,
  ...
  tbb::parallel_sort(indices.begin(), indices.end(), [&](int i, int j) {
    return cloud->at(i).curvature < cloud->at(j).curvature;
  });
  ...
  auto monitor = spdmon::LoggerProgress("Region Growing", total);
  spdlog::stopwatch sw;
  while (indices.size() > 0 && indices.size() > total * early_stop) {
    ...
    // Cluster indices, indecies of points in the point cloud that are part of
    // the cluster
...
#pragma omp parallel for reduction(merge : results)
      for (size_t i = 0; i < front.size(); ++i) {
        ...
        std::copy_if(
            nn_indices.begin(), nn_indices.end(), std::back_inserter(temp),
            [&](int i) { return index_map.find(i) != index_map.end(); });
...
#pragma omp parallel for
        for (size_t i = 0; i < nn_indices.size(); ++i) {
          ...
          Eigen::Vector3f p;
          pcl::geometry::project(cloud->at(nn_indices[i]).getVector3fMap(),
                                 plane_origin, plane_normal, p);
      ...
      for (auto i : results) {
        ...
        // TODO: Use the return value to check if the index needs to be inserted
        // in to the front
      ...
      if (cluster_indices.size() > next_update) {
        pcl::Indices indices_local(cluster_indices.begin(),
                                   cluster_indices.end());
    ...
    monitor += cluster_indices.size();
  }
  spdlog::info("Region growing took {} s", sw);
