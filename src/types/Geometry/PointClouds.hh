...
/// Point clouds can be a clollection of point cloud files, or in memory point
/// clouds.
/// @tparam T The type underlying type either `std::string` or `PointCloud`
template <typename T> class PointClouds {
public:
...
#pragma omp parallel for shared(files, data)
  ...
  void resize(typename DataT::size_type size) { data.resize(size); }
  void reserve(typename DataT::size_type size) { data.reserve(size); }
  typename DataT::iterator begin() { return data.begin(); }
  typename DataT::iterator end() { return data.end(); }
