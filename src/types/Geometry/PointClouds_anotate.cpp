...
static void draw_box(cv::Mat &img, ReUseX::OutputParams const &param) {
  ...
  cv::putText(img, label, cv::Point(param.box.x, param.box.y),
              cv::FONT_HERSHEY_SIMPLEX, 1, color_scalar, 2);
  ...
  cv::drawContours(overlay(param.box), contours, -1, color_scalar, cv::FILLED,
...
static constexpr std::uint32_t Deselected =
    std::numeric_limits<std::uint32_t>::max() - 1;

namespace ReUseX {
template <class T>
PointClouds<T> PointClouds<T>::annotate(std::string yolo_path,
  ...
  if constexpr (std::is_same<T, std::string>::value) {
    std::cout << "Not checking if point cloud is dense for On disk point clouds"
              << std::endl;
  ...
  monitor = std::make_shared<spdmon::LoggerProgress>("Loading", n_frames);
...
#pragma omp parallel for shared(data, input_images)
  ...
  spdlog::info("Loading took {} s", sw1);
  ...
  monitor.reset();
  monitor = std::make_shared<spdmon::LoggerProgress>("Preprocessing", n_frames);
...
#pragma omp parallel for shared(input_images, blobs, params)
  ...
  spdlog::info("Preprocessing took {} s", sw2);
  ...
  monitor.reset();
  monitor = std::make_shared<spdmon::LoggerProgress>("Inference", n_frames);
  spdlog::stopwatch sw3;
  for (size_t i = 0; i < n_frames; i++) {
    outputs[i] = model.Detect(blobs[i]);
    ++(*monitor);
  }
  spdlog::info("Inference took {} s", sw3);
  ...
  monitor.reset();
  monitor =
      std::make_shared<spdmon::LoggerProgress>("Postprocessing", n_frames);
  spdlog::stopwatch sw4;
#pragma omp parallel for shared(data, outputs, params, input_images)
  for (size_t i = 0; i < n_frames; i++) {

    auto results =
        Yolov8Seg::Postprocess(outputs[i], params[i], input_images[i].size());
...
#pragma omp parallel for
    ...
    // cv::Size input_image_size_right_side_up = input_images[i].size();//
    // cv::Size(input_image_size.height, input_image_size.width);
    cv::Size input_image_size =
        cv::Size(input_images[i].size().width, input_images[i].size().height);

    // auto img = cloud->image();
    // cv::cvtColor(img, img, cv::COLOR_RGB2BGR);

    for (OutputParams const &result_params : results) {
      OutputParams result_param_r =
          result_params
              .Scale(input_image_size,
                     cv::Size(cloud_size.height, cloud_size.width))
...
#pragma omp parallel for collapse(2) shared(cloud, result_param_r)             \
    firstprivate(row_start, row_end, col_start, col_end)
      for (int y = row_start; y < row_end; y++) {
        for (int x = col_start; x < col_end; x++) {
          if (result_param_r.boxMask.at<uchar>(y - row_start, x - col_start) >
              0.5)
  ...
  spdlog::info("Postprocessing took {} s", sw4);
...
PointClouds<T> PointClouds<T>::annotate_from_hdf5(std::string hdf5_path) {
  ...
  // // Sort the frames
  // std::sort(frames.begin(), frames.end(), [ ](const std::string &a, const
  // std::string &b) {
...
template PointCloudsInMemory
PointCloudsInMemory::annotate(std::string, std::optional<Dataset> &);
template PointCloudsOnDisk
PointCloudsOnDisk::annotate(std::string, std::optional<Dataset> &);

template PointCloudsOnDisk PointCloudsOnDisk::annotate_from_hdf5(std::string);
template PointCloudsInMemory
    PointCloudsInMemory::annotate_from_hdf5(std::string);
