...
template <> std::string merge(std::string left, std::string right) {
...
template <> PointCloud merge(PointCloud left, PointCloud right) {
...
inline void merge_clouds(PointCloud lhs, const PointCloud rhs) { *lhs += *rhs; }

#pragma omp declare reduction(+ : PointCloud : merge_clouds(omp_out, omp_in))  \
    initializer(omp_priv = PointCloud())

template <typename T> PointCloud PointClouds<T>::merge() {
  ...
  auto monitor = spdmon::LoggerProgress("Merging", (data.size() - 1));
...
#pragma omp parallel for reduction(+ : cloud)
    ...
    while (data.size() != 1) {
      ...
      std::cout << "Loop: " << loop_count << ", #Elements: " << data.size()
                << std::endl;

      std::vector<std::string> new_paths(data.size(), "");

      size_t threads =
          (data.size() * GROUP_SIZE > (size_t)omp_get_max_threads())
...
#pragma omp parallel for shared(data, new_paths)
      for (std::size_t i = 0; i < data.size(); i += GROUP_SIZE) {
        ...
        // #pragma omp parallel for reduction(+:merged)
        for (std::size_t j = 1 /*Don't remove the file we just saved*/;
             j < GROUP_SIZE; ++j) {
          if (i + j < data.size()) {
            std::filesystem::remove(data[i + j]);
            auto header =
                std::filesystem::path(data[i + j]).replace_extension(".head");
      ...
      data.clear();
      std::copy_if(new_paths.begin(), new_paths.end(), std::back_inserter(data),
                   [](std::string path) { return path != ""; });
      int diff = total_before - data.size();

      std::cout << "Merging done! Total before: " << total_before
                << ", Total after: " << data.size() << ", Diff: " << diff
                << std::endl;
...
#pragma omp parallel for
      ...
      spdlog::info("Downsampling took: {} seconds", sw2);
  ...
  spdlog::info("Merging took: {} seconds", sw1);
