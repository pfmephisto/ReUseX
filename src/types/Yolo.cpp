...
template <>
OutputParams OutputParams::Rotate<cv::RotateFlags::ROTATE_90_CLOCKWISE>(
    cv::Size size) const {
  OutputParams result;
  if (!valid) {
  ...
  // rect [x,y,w,h]
  result.box =
      cv::Rect(size.height - box.y - box.height, box.x, box.height, box.width);

  // rect [x,y,w,h]
  cv::Point center =
      cv::Point(size.width - rotatedBox.center.y, rotatedBox.center.x);
  result.rotatedBox =
      cv::RotatedRect(center, rotatedBox.size, rotatedBox.angle + 90);
...
template <>
OutputParams OutputParams::Rotate<cv::RotateFlags::ROTATE_90_COUNTERCLOCKWISE>(
    cv::Size size) const {
  OutputParams result;
  if (!valid) {
    // Skip rotating if the result is invalid
  ...
  // rect [x,y,w,h]
  result.box =
      cv::Rect(box.y, size.width - box.x - box.width, box.height, box.width);

  // rect [x,y,w,h]
  cv::Point center =
      cv::Point(rotatedBox.center.y, size.height - rotatedBox.center.x);
  result.rotatedBox =
      cv::RotatedRect(center, rotatedBox.size, rotatedBox.angle - 90);

  cv::Mat rt_boxMask;
  cv::rotate(boxMask, result.boxMask,
...
template <>
OutputParams
OutputParams::Rotate<cv::RotateFlags::ROTATE_180>(cv::Size size) const {
  OutputParams result;
  if (!valid) {
    // Skip rotating if the result is invalid
    return result;
  ...
  // rect [x,y,w,h]
  result.box = cv::Rect(size.height - box.y - box.height,
                        size.width - box.x - box.width, box.width, box.height);

  // rect [x,y,w,h]
  cv::Point center = cv::Point(size.width - rotatedBox.center.x,
                               size.height - rotatedBox.center.y);
  result.rotatedBox =
      cv::RotatedRect(center, rotatedBox.size, rotatedBox.angle);
...
//    bool autoShape, bool scaleFill, bool scaleUp, int stride, const
//    cv::Scalar& color)
...
// void DrawPred(Mat& img, vector<OutputSeg> result, std::vector<std::string>
// classNames, vector<Scalar> color, bool isVideo) {
//     Mat mask = img.clone();
...
//             mask(result[i].box).setTo(color[result[i].id],
//             result[i].boxMask);
//         string label = classNames[result[i].id] + ":" +
//         to_string(result[i].confidence); int baseLine; Size labelSize =
//         getTextSize(label, FONT_HERSHEY_SIMPLEX, 0.5, 1, &baseLine); top =
...
//         255, 0), FILLED); putText(img, label, Point(left, top),
//         FONT_HERSHEY_SIMPLEX, 1, color[result[i].id], 2);
//     }
...
void Yolov8Seg::Preprocess(cv::Mat &srcImg, cv::Mat &blob, cv::Vec4d &params) {
  ...
  cv::dnn::blobFromImage(netInputImg, blob, 1.0 / 255,
                         cv::Size(_netWidth, _netHeight), cv::Scalar(0, 0, 0),
                         true, false);
}

std::vector<OutputParams>
Yolov8Seg::Postprocess(const std::vector<cv::Mat> &blob,
                       const cv::Vec4d &params, const cv::Size &srcImgSize) {
  return ReUseX::Postprocess(blob, params, srcImgSize, _classThreshold,
                             _nmsThreshold, _maskThreshold, _netWidth,
                             _netHeight);
}

std::vector<OutputParams>
Postprocess(const std::vector<cv::Mat> &blob, const cv::Vec4d &params,
            const cv::Size &srcImgSize, const float classThreshold,
            const float nmsThreshold, const float maskThreshold,
  ...
  std::vector<std::vector<float>>
      picked_proposals; // output0[:,:, 4 + _className.size():net_width]===> for
                        // mask
  cv::Mat output0 = cv::Mat(cv::Size(blob[0].size[2], blob[0].size[1]), CV_32F,
                            (float *)blob[0].data)
  ...
  cv::dnn::NMSBoxes(boxes, confidences, classThreshold, nmsThreshold,
                    nms_result);
  ...
  mask_params.netHeight = netHeight;
