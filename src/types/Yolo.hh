...
struct OutputParams {
  ...
  cv::Mat boxMask; // Mask within the rectangular frame to save memory space and
                   // speed up
  bool valid = true; // Whether the result is valid
  template <cv::RotateFlags flag> OutputParams Rotate(cv::Size size) const;
  ...
  OutputParams Scale(cv::Size size_in, cv::Size size_out) const {
    ...
    // rect [x,y,w,h]
    result.box = cv::Rect(box.x * scale_x, box.y * scale_y, box.width * scale_x,
                          box.height * scale_y);

    // rect [x,y,w,h]
    cv::Point center =
        cv::Point(rotatedBox.center.x * scale_x, rotatedBox.center.y * scale_y);
    cv::Size size = cv::Size(0, 0);
    result.rotatedBox = cv::RotatedRect(center, size, rotatedBox.angle);

    cv::Size size_target =
        cv::Size(boxMask.cols * scale_x, boxMask.rows * scale_y);
...
public:
  ...
  static std::vector<OutputParams> Postprocess(const std::vector<cv::Mat> &blob,
