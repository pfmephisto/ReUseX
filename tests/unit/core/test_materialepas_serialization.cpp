...
void verifyPassportEquality(const MaterialPassport &original,
  ...
  // Section 2: Construction Item Description
  REQUIRE(retrieved.description.designation ==
          original.description.designation);
  REQUIRE(retrieved.description.images == original.description.images);
  REQUIRE(retrieved.description.has_qr_code ==
          original.description.has_qr_code);
  REQUIRE(retrieved.description.has_rfid_tag ==
          original.description.has_rfid_tag);
  REQUIRE(retrieved.description.materials == original.description.materials);
  REQUIRE(retrieved.description.assembly_methods ==
          original.description.assembly_methods);
  REQUIRE(retrieved.description.year_of_installation ==
          original.description.year_of_installation);
  REQUIRE(retrieved.description.year_of_construction ==
  ...
  REQUIRE(retrieved.product.production_year ==
          original.product.production_year);

  // Section 4: Certifications (13 properties)
  REQUIRE(retrieved.certifications.has_epd == original.certifications.has_epd);
  REQUIRE(retrieved.certifications.epd_programme_operator ==
          original.certifications.epd_programme_operator);
  REQUIRE(retrieved.certifications.epd_operator_web_domain ==
          original.certifications.epd_operator_web_domain);
  REQUIRE(retrieved.certifications.epd_registration_number ==
          original.certifications.epd_registration_number);
  REQUIRE_THAT(
      retrieved.certifications.reference_service_life.value(),
      WithinAbs(original.certifications.reference_service_life.value(), 0.001));
  REQUIRE(retrieved.certifications.has_safety_data_sheet ==
          original.certifications.has_safety_data_sheet);
  REQUIRE(retrieved.certifications.declaration_of_performance ==
          original.certifications.declaration_of_performance);
  REQUIRE(retrieved.certifications.technical_documentation ==
          original.certifications.technical_documentation);
  REQUIRE(retrieved.certifications.non_destructive_tests ==
          original.certifications.non_destructive_tests);
  REQUIRE_THAT(
      retrieved.certifications.assessed_period_of_use.value(),
      WithinAbs(original.certifications.assessed_period_of_use.value(), 0.001));
  REQUIRE_THAT(
      retrieved.certifications.avg_service_life_build.value(),
      WithinAbs(original.certifications.avg_service_life_build.value(), 0.001));
  REQUIRE_THAT(
      retrieved.certifications.remaining_service_life_rsl.value(),
      WithinAbs(original.certifications.remaining_service_life_rsl.value(),
                0.001));
  REQUIRE_THAT(
      retrieved.certifications.remaining_service_life_build.value(),
      WithinAbs(original.certifications.remaining_service_life_build.value(),
                0.001));

  // Section 5: Dimensions (8 properties)
  REQUIRE_THAT(retrieved.dimensions.width_mm.value(),
               WithinAbs(original.dimensions.width_mm.value(), 0.001));
  REQUIRE_THAT(retrieved.dimensions.height_mm.value(),
               WithinAbs(original.dimensions.height_mm.value(), 0.001));
  REQUIRE_THAT(retrieved.dimensions.length_mm.value(),
               WithinAbs(original.dimensions.length_mm.value(), 0.001));
  REQUIRE_THAT(retrieved.dimensions.thickness_mm.value(),
               WithinAbs(original.dimensions.thickness_mm.value(), 0.001));
  REQUIRE_THAT(retrieved.dimensions.weight_kg.value(),
               WithinAbs(original.dimensions.weight_kg.value(), 0.001));
  REQUIRE_THAT(retrieved.dimensions.volume_m3.value(),
               WithinAbs(original.dimensions.volume_m3.value(), 0.001));
  REQUIRE_THAT(retrieved.dimensions.surface_area_m2.value(),
               WithinAbs(original.dimensions.surface_area_m2.value(), 0.001));
  REQUIRE(retrieved.dimensions.technical_drawing ==
          original.dimensions.technical_drawing);

  // Section 6: Condition (8 properties)
  REQUIRE(retrieved.condition.photo_documentation ==
          original.condition.photo_documentation);
  REQUIRE(retrieved.condition.visual_inspection_performed ==
          original.condition.visual_inspection_performed);
  REQUIRE(retrieved.condition.has_signs_of_damage ==
  ...
  REQUIRE(retrieved.condition.is_surface_intact ==
          original.condition.is_surface_intact);
  REQUIRE(retrieved.condition.has_intact_edges ==
          original.condition.has_intact_edges);
  REQUIRE(retrieved.condition.has_signs_of_degradation ==
          original.condition.has_signs_of_degradation);

  // Section 7: Pollution (with nested objects)
  REQUIRE(retrieved.pollution.contains_reach_substances ==
          original.pollution.contains_reach_substances);
  REQUIRE(retrieved.pollution.is_chemically_treated ==
          original.pollution.is_chemically_treated);
  REQUIRE(retrieved.pollution.surface_treatments ==
          original.pollution.surface_treatments);
  REQUIRE(retrieved.pollution.intended_for_indoor_use ==
          original.pollution.intended_for_indoor_use);
  REQUIRE(retrieved.pollution.labelling_scheme ==
          original.pollution.labelling_scheme);
  REQUIRE(retrieved.pollution.emission_level ==
          original.pollution.emission_level);
  REQUIRE(retrieved.pollution.has_asbestos_analysis ==
          original.pollution.has_asbestos_analysis);

  // Verify nested DangerousSubstance array
  REQUIRE(retrieved.pollution.dangerous_substances.size() ==
  ...
  REQUIRE(retrieved.pollution.emissions.size() ==
  ...
  for (size_t i = 0; i < original.pollution.emissions.size(); ++i) {
    ...
    REQUIRE_THAT(retr_em.lower_interval.value(),
                 WithinAbs(orig_em.lower_interval.value(), 0.001));
    REQUIRE_THAT(retr_em.upper_interval.value(),
                 WithinAbs(orig_em.upper_interval.value(), 0.001));
    REQUIRE(retr_em.quantity_type == orig_em.quantity_type);
    REQUIRE_THAT(retr_em.quantity.value(),
                 WithinAbs(orig_em.quantity.value(), 0.001));
  }

  // Section 8: Environmental Potential
  REQUIRE(retrieved.environmental.takeback_scheme_available ==
          original.environmental.takeback_scheme_available);
  REQUIRE(retrieved.environmental.consists_of_separate_parts ==
          original.environmental.consists_of_separate_parts);

  // Section 9: Fire Properties
  REQUIRE(retrieved.fire.reaction_to_fire == original.fire.reaction_to_fire);
  REQUIRE(retrieved.fire.resistance_to_fire ==
          original.fire.resistance_to_fire);
  REQUIRE(retrieved.fire.documentation_of_fire_classification ==
          original.fire.documentation_of_fire_classification);
  REQUIRE(retrieved.fire.field_of_application ==
          original.fire.field_of_application);

  // Section 10: History
  REQUIRE(retrieved.history.previous_usage_environments ==
  ...
  REQUIRE(retrieved.metadata.version_number ==
          original.metadata.version_number);
  ...
  for (size_t i = 0; i < original.transaction_log.size(); ++i) {
    REQUIRE(retrieved.transaction_log[i].type ==
            original.transaction_log[i].type);
    REQUIRE(retrieved.transaction_log[i].guid ==
            original.transaction_log[i].guid);
    REQUIRE(retrieved.transaction_log[i].edited_by ==
            original.transaction_log[i].edited_by);
    REQUIRE(retrieved.transaction_log[i].edited_date ==
            original.transaction_log[i].edited_date);
    REQUIRE(retrieved.transaction_log[i].old_value ==
            original.transaction_log[i].old_value);
    REQUIRE(retrieved.transaction_log[i].new_value ==
            original.transaction_log[i].new_value);
  }
}
...
TEST_CASE("MaterialPassport_NonExistentGuid_ThrowsRuntimeError",
  ...
  {
    reusex::ProjectDB db(temp_db, true);
    REQUIRE_THROWS_AS(db.material_passport("non-existent-guid"),
                      std::runtime_error);
  }
...
TEST_CASE("MaterialPassport_MultiplePassports_RoundTripIndependently",
  ...
  // Write all three passports
  {
    ...
    db.add_material_passport(passport3,
                             "project-A"); // Same project as passport1
  }
